...
`List`适用于量有限且无缓存的单一集合数据查询；若需要更广的应用，**应该**[用自定义方法](05-定制方法.md)`Search`。
...
使用常见模式：[分页](09-通用设计模式.md#列表分页)，[结果排序](https://cloud.google.com/apis/design/design_patterns#sorting_order)。
...
* =请求消息字段中与被查询的资源名称对应的**应该**映射到URL路径，URL模板中的最后一段（即集合ID）**必须**为文字。==
...
* 标准的`Update`方法**应该**支持部分资源更新，并使用 HTTP `PATCH`方法以及名为update_mask的FieldMask字段。
