...
\*\*Delete方法如果并没有立刻删除响应的资源（例如创建一个耗时删除操作或者更新标识），它的响应**应该**包括耗时操作（译者注：耗时操作可看做是对远程操作的一个抽象。之所以需要这个，是因为有些远程的运行过程耗时比较长。所以先给调用方返回一个对象，这个对象对应一个远程的执行过程，可以用它获取远程操作的状态和结果）或更新后的资源。
